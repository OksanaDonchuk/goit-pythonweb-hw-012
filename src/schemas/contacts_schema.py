...
class ContactSchema(BaseModel):
    ...
    Використовується при створенні нового запису у БД.
    Забезпечує валідацію даних:
    - мінімальна/максимальна довжина імені та прізвища,
    - коректність email,
    - формат телефону,
    - перевірка дати народження (не може бути у майбутньому).
    """
...
class ContactUpdateSchema(BaseModel):
    ...
    Дозволяє змінювати будь-які поля контакту (всі необов’язкові).
    Включає:
    - нормалізацію рядків (обрізка пробілів, email у нижньому регістрі),
    - перевірку, що день народження не в майбутньому,
    - валідацію: хоча б одне поле повинно бути передано.
    """
...
class ContactResponse(BaseModel):
    ...
    Використовується для повернення контактів з бази:
    - містить всі основні поля (ім’я, email, телефон),
    - додаткову інформацію, дату створення та оновлення.
    """
